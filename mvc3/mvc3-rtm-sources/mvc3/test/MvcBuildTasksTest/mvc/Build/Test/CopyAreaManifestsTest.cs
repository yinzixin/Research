...
<AreaInfo xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"">
  ...
  <Type>Content</Type>
        ...
        private static void ExpectCreateDirectory(Mock<IFileSystem> mockFs, string path) {
            mockFs.Setup(o => o.CreateDirectory(path)).Verifiable();
        }

        private static void ExpectFileCopy(Mock<IFileSystem> mockFs, string source, string destination) {
            mockFs.Setup(o => o.FileCopy(source, destination, true)).Verifiable();
        }

        private static IReturnsResult<IFileSystem> ExpectFileList(Mock<IFileSystem> mockFs, params string[] files) {
            return mockFs.Setup(o => o.GetFiles(_manifestPath, "*.area-manifest.xml", SearchOption.TopDirectoryOnly)).Returns(files);
        }

        private static IReturnsResult<IFileSystem> ExpectFileExists(Mock<IFileSystem> mockFs, string fileName, bool exists) {
            return mockFs.Setup(o => o.FileExists(fileName)).Returns(exists);
        }

        private static IReturnsResult<IFileSystem> ExpectReadFile(Mock<IFileSystem> mockFs, string fileName, string fileContents) {
            return mockFs.Setup(o => o.FileOpen(fileName, FileMode.Open)).Returns(CreateMemoryStream(fileContents));
        }
