namespace System.Web.Mvc.Test {
    ...
    public class ControllerActionInvokerTest {
        ...
        public void GetFiltersSplitsFilterObjectsIntoFilterInfo() {
            ...
            // Assert
            Assert.AreSame(actionFilter, result.ActionFilters.Single());
            Assert.AreSame(authFilter, result.AuthorizationFilters.Single());
            Assert.AreSame(exFilter, result.ExceptionFilters.Single());
            Assert.AreSame(resultFilter, result.ResultFilters.Single());
        }
        ...
        public void InvokeActionCallsValidateRequestIfAsked() {
            ...
            // Assert
            Assert.IsTrue(validateInputWasCalled, "ValidateInput was not called.");
            mockHelper.Verify();
        }
        ...
        public void InvokeActionInvokesExceptionFiltersAndRethrowsExceptionIfNotHandled() {
            ...
            mockHelper.Setup(h => h.PublicInvokeAuthorizationFilters(context, filterInfo.AuthorizationFilters, ad)).Throws(exception).Verifiable();
            mockHelper.Setup(h => h.PublicInvokeExceptionFilters(context, filterInfo.ExceptionFilters, exception)).Returns(exContext).Verifiable();
            mockHelper.Setup(h => h.PublicInvokeActionResult(context, It.IsAny<ActionResult>())).Callback(delegate {
        ...
        public void InvokeExceptionFilters() {
            ...
            Assert.AreEqual(2, callQueue.Count);
            Assert.AreSame(filter2, callQueue[0]);  // Exception filters are executed in reverse order
            Assert.AreSame(filter1, callQueue[1]);
        }
