...
namespace System.Web.Helpers {
    ...
    public class WebImage {
        ...
        private PropertyItem[] _properties = null; // image metadata
        ...
        internal WebImage(HttpContextBase httpContext, string filePath) {
            if (String.IsNullOrEmpty(filePath)) {
                throw new ArgumentException(CommonResources.Argument_Cannot_Be_Null_Or_Empty, "filePath");
            }

            _fileName = filePath;
            _content = File.ReadAllBytes(VirtualPathUtil.MapPath(httpContext, filePath));
        ...
        public WebImage AddImageWatermark(
                ...
                int opacity = 100,
                int padding = 5) {
            return AddImageWatermark(new HttpContextWrapper(HttpContext.Current), watermarkImageFilePath, width, height,
        ...
        internal WebImage AddImageWatermark(
                ...
                int opacity,
                int padding) {
            return AddImageWatermark(new WebImage(httpContext, watermarkImageFilePath), width, height, horizontalAlign, verticalAlign, opacity, padding);
        }
        ...
        /// <param name="forceWellKnownExtension">Appends a well known extension to the filePath based on the imageFormat specified. 
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Globalization", "CA1308:NormalizeStringsToUppercase", Justification="The string is a file extension which is typically lower case")]
        ...
        private void GetContentFromImageAndUpdateFormat(Image image) {
            using (MemoryStream buffer = new MemoryStream()) {
                // Save method throws nonsensical exception when ImageFormat is MemoryBmp.
                if (image.RawFormat.Equals(System.Drawing.Imaging.ImageFormat.MemoryBmp)) {
                    image.Save(buffer, System.Drawing.Imaging.ImageFormat.Bmp);
                    this._currentFormat = System.Drawing.Imaging.ImageFormat.Bmp;
                }
                else {
        ...
        private void ApplyTransformationsAndSetProperties() {
            ...
            try {
                ...
                // If there were any transformations we need to get new content.
                // This will also set image format.
                if (_transformations.Any()) {
        ...
        private class CropTransformation : ImageTransformation {
            ...
            public override Image ApplyTransformation(Image image) {
                ...
                int width = image.Width - (this.Left + this.Right);
                int height = image.Height - (this.Top + this.Bottom);

                RectangleF rect = new RectangleF(this.Left, this.Top, width, height);
        ...
        private abstract class WatermarkTransformation : ImageTransformation {
            ...
            public Rectangle GetRectangleInsideImage(Image image, int width, int height) {
                ...
                switch (this.HorizontalAlign) {
                    ...
                    case HorizontalAlign.Right:
                        posX = image.Width - width - this.Padding;
                        break;
                ...
                switch (this.VerticalAlign) {
                    ...
                    case VerticalAlign.Bottom:
                        posY = image.Height - height - this.Padding;
                        break;
            ...
            private static float[][] GetScalingMatrix(float alphaScaling) {
                ...
                float[][] scalingMatrix ={ 
                    ...
                    new float[] {0, 1, 0, 0, 0},
                    new float[] {0, 0, 1, 0, 0},
                    new float[] {0, 0, 0, alphaScaling, 0}, 
                    new float[] {0, 0, 0, 0, 1}};
        ...
        private class WatermarkTextTransformation : WatermarkTransformation {
            ...
            public override Image ApplyTransformation(Image image) {
                ...
                if ((this.Padding * 2 >= image.Width) || (this.Padding * 2 >= image.Height)) {
                ...
                using (Bitmap textBitmap = new Bitmap(textWidth, textHeight)) {
                    using (Graphics graphics = Graphics.FromImage(textBitmap)) {
                        using (Font font = new Font(this.FontFamily, fontSize, this.FontStyle)) {
                            using (Brush brushToUse = new SolidBrush(this.FontColor)) {
                                graphics.TextRenderingHint = System.Drawing.Text.TextRenderingHint.AntiAlias;
                                graphics.DrawString(this.Text, font, brushToUse, new PointF(0F, 0F));
                            }
                        }
                    }
            ...
            private int GetBestFontSize(Image image, Graphics graphics, out SizeF textArea) {
                ...
                SizeF layoutArea = new SizeF(image.Width - this.Padding * 2, image.Height - this.Padding * 2);
                ...
                using (StringFormat format = new StringFormat(StringFormatFlags.NoClip | StringFormatFlags.MeasureTrailingSpaces)) {
                    for (int fontSize = this.FontSize; fontSize >= 2; fontSize--) {
                        int numChars = 0, numLines = 0;
                        using (Font font = new Font(this.FontFamily, fontSize, this.FontStyle)) {
                            textArea = graphics.MeasureString(this.Text, font, layoutArea, format, out numChars, out numLines);
                        }

                        if ((numChars >= this.Text.Length) && (textArea.Width <= layoutArea.Width) && (textArea.Height <= layoutArea.Height)) { // it fits! Exit now
                            return fontSize;
                        }
                        else {
        ...
        private class WatermarkImageTransformation : WatermarkTransformation {
            ...
            public override Image ApplyTransformation(Image image) {
                ...
                if (this.Width == 0) {
                    Debug.Assert(this.Height == 0, "If one dimension is zero the other one must be too.");
                    this.Width = this.WatermarkImage.Width;
                    this.Height = this.WatermarkImage.Height;
                }

                if ((this.Padding * 2 + this.Width >= image.Width) || (this.Padding * 2 + this.Height >= image.Height)) {
                    // If watermark image + padding is too big we don't make any changes. 
                    return image;
                }

                this.WatermarkImage.Resize(this.Width, this.Height, preserveAspectRatio: false, preventEnlarge: false);
                float alphaScaling = ((float)this.Opacity) / 100;

                byte[] watermarkBuffer = this.WatermarkImage.GetBytes();
                Rectangle rect = GetRectangleInsideImage(image, this.Width, this.Height);
