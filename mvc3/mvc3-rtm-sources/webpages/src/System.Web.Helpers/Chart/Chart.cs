...
namespace System.Web.Helpers {
    ...
    public class Chart {
        ...
        private class DataSourceData {
            ...
            public DV.PointSortOrder PointSortOrder { get; set; }
        ...
        private class SeriesData {
            ...
            public DV.SeriesChartType ChartType { get; set; }
        ...
        [SuppressMessage("Microsoft.Naming", "CA1704:IdentifiersShouldBeSpelledCorrectly", MessageId = "x", Justification = "Name based on X-axis")]
        [SuppressMessage("Microsoft.Naming", "CA1704:IdentifiersShouldBeSpelledCorrectly", MessageId = "y", Justification = "Name based on Y-axis")]
        public Chart AddSeries(
            ...
            _series.Add(new SeriesData() {
                Name = name,
                ChartType = ConvertStringArgument<DV.SeriesChartType>("chartType", chartType),
                ChartArea = chartArea,
                AxisLabel = axisLabel,
                Legend = legend,
        ...
        public Chart DataBindCrossTable(IEnumerable dataSource, string groupByField, string xField, string yFields,
            ...
            _dataSource = new DataSourceData() {
                ...
                PointSortOrder = ConvertStringArgument<DV.PointSortOrder>("pointSortOrder", pointSortOrder)
            };
            return this;
        }
        ...
        internal void ExecuteChartAction(Action<DV.Chart> action) {
            using (DV.Chart chart = new DV.Chart()) {
        ...
        private void LoadThemes(DV.Chart chart) {
        ...
        private static void LoadChartThemeFromFile(DV.Chart chart, Stream templateStream) {
            ...
            chart.Serializer.Content = DV.SerializationContents.All;
        ...
        private void ApplySeries(DV.Chart chart) {
            foreach (var seriesData in _series) {
                var series = new DV.Series();
                try {
        ...
        private void ApplyTitles(DV.Chart chart) {
            foreach (var titleData in _titles) {
                var title = new DV.Title();
                try {
        ...
        /// </summary>
        private static DV.ChartImageFormat ConvertStringToChartImageFormat(string format) {
            object result;
            format = NormalizeFormat(format);
            if (!ConversionUtil.TryFromString(typeof(DV.ChartImageFormat), format, out result)) {
                throw new ArgumentException(String.Format(CultureInfo.CurrentCulture,
                    HelpersResources.Image_IncorrectImageFormat, format), "format");
            }
            return (DV.ChartImageFormat)result;
        }
