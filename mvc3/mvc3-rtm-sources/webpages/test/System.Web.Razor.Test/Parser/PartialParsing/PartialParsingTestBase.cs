...
namespace System.Web.Razor.Test.Parser.PartialParsing {
    public abstract class PartialParsingTestBase<TLanguage> where TLanguage : RazorCodeLanguage, new() {
        ...
        protected static void RunPartialParseTest(TextChange change, Block newTreeRoot, PartialParseResult additionalFlags = (PartialParseResult)0) {
            ...
            // Assert
            Assert.AreEqual(PartialParseResult.Accepted | additionalFlags, result, "The specified change caused a structural change");
            Assert.AreEqual(1, manager.ParseCount, "An unexpected reparse occurred");
